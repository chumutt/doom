...
** remove all blank lines custom script :custom:
#+begin_src elisp
...
#+end_src
*** Custom Command to Show Only DONE Tasks Today
#+begin_src elisp
...
#+end_src
** TODO turn ledger-mode into hledger-mode

figuratively, of course

requires hledger to be installed, obviously
...
#+end_src
*** hledger.sh script
#+begin_src sh :tangle ~/.local/bin/hledger.sh :shebang "#!/bin/bash"
# hledger.sh
...
*** TODO further ledger-mode configuration
**** TODO Use strict account name checking.
#+begin_src elisp
;; (after! 'ledger-mode
...
(defun chu/ledger-post-fill (&optional beg end)
  "Fill missing amounts in current transaction or in region if active."
  (interactive (if (use-region-p)
                   (list (region-beginning) (region-end))))
  (save-excursion
    (if (and beg end)
        ;; Region-based: iterate over transactions
        (progn
          (goto-char beg)
          (while (and (< (point) end)
      ...
      (chu/ledger-fill-1-xact))))
#+end_src
** TODO csv to ledger (c2l) config :ledger:pta:
#+begin_src elisp
...
org-tanglesync for doom emacs! be careful
...
#+begin_example elisp
(setq! org-tanglesync-watch-files '("~/Nextcloud/documents/org/example.org"))
#+end_example
** IDEA workspaces initialization
#+begin_example elisp
(+workspace/new-named "agender")
(org-agenda-list)
(+workspace/new-named "todo")
(+workspace/new-named "daily")
(+workspace/new-named "ledger")
(+workspace/new)
#+end_example
...
since this is the 'navi' branch this is enabled by default:
#+begin_src elisp :tangle no
;; (defvar IS-WSL t)
#+end_src
