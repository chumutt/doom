...
#+begin_src elisp
;; Guard recentf-exclude against early references before recentf loads
(when (boundp 'recentf-exclude)
  (setq recentf-exclude (append recentf-exclude '("TAGS" "/tmp/" "\\.gpg$"))))
#+end_src

#+begin_src elisp
;; Prevent winner-ring from breaking workspace/persp saves
(after! persp-mode
  (remove-hook 'persp-save-state-functions #'+workspace--save-winner-ring)
  (remove-hook 'persp-save-state-functions #'persp-save-winner-ring))
#+end_src
...
*** Custom function for linking latest screenshot as captured by Windows-to-WSL2 Screenshot Bridge

=latest.png= is a symbolic link to whichever screenshot was taken most recently, which will then be copied to the user's attachment directory for Emacs org-mode.
...
*** Org latex preview hugo fix attempt
...
***** NO An even newer version :@computer:@home:@programming:

#+begin_src elisp :tangle no
(setq org-capture-templates
  '(
    ("t" "Personal todo / tasks")
    ...
    ("tt" "Task" entry
     (
      file+headline +org-capture-todo-file
      "Inbox")
     "* TODO %?\n  %U\n  %a\n  %i"
     :empty-lines 1)
...
#+begin_src elisp
;; Drop empty-identity snippets that trigger yas warnings
(after! yasnippet
  (defun chu/clean-empty-yas-snippets ()
    (when (boundp 'yas--snippets-table)
      (setq yas--snippets-table
            (cl-delete-if (lambda (s) (string= (yas--snippet-name s) ""))
                          (hash-table-values yas--snippets-table)))))
  (add-hook 'yasnippet-snippet-mode-hook #'chu/clean-empty-yas-snippets))
#+end_src
...
I wouldn't bother using anything other than the "base.en" model specified below by the two model and language variables.
#+begin_src elisp
(use-package whisper
  :config
...
**** custom hledger formatting scripts (example)
...
*** Set default currency string in =hledger=
#+begin_src elisp
(setq hledger-currency-string "$")
#+end_src
*** Balance transaction at point / in selected region

#+name: hledger-balance-transaction
#+begin_src elisp
...
Otherwise operate on transaction at point."
  ...
  (save-excursion
    ...
    (while (< (point) end)
      ...
      (forward-line 1))))
#+end_src
*** TODO Doom-native formatter for hledger

#+begin_src elisp
#+end_src

**** TODO Enable format-on-save for hledger/ledger modes
...
#+begin_src elisp
(defvar IS-WSL
  (when-let* ((uname (shell-command-to-string "uname -a")))
...
*** DONE share clipboard between windows host and WSL2
:PROPERTIES:
...
** Set safe local variable values for exports (Hugo/ox-hugo)

#+begin_src elisp
(setq safe-local-variable-values '((eval org-hugo-auto-export-mode t)))
#+end_src

** Set ignored local variable values for exports (Org)
