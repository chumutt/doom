...
Ensure that blank lines exist between headings and between headings and their contents.  With prefix (see ~prefix-arg~ emacs variable), operate on whole buffer.  Ensures that blank lines exist after each headings's drawers.
